...
export const ClientDetailPanel: React.FC<ClientDetailPanelProps> = ({
  ...
  const calcularRetornoPercentual = (
    fundos: number | null | undefined,
    ...
    <>
      <div className="relative flex h-full min-h-0 flex-col bg-white">
          ...
          <div
            className={
              compact
                ? 'flex justify-between items-start gap-2 px-3 py-2 border-b border-slate-200 flex-shrink-0'
                : isMobile
                ? 'flex justify-between items-start gap-2 px-4 pb-3 border-b border-slate-200 flex-shrink-0'
                : 'flex justify-between items-start px-6 pt-6 pb-4 border-b border-slate-100 flex-shrink-0'
            }
            style={!compact && isMobile ? { paddingTop: 'calc(0.75rem + env(safe-area-inset-top))' } : undefined}
          >
              <div className="flex-1 min-w-0">
                <div className="flex items-center gap-2">
                  <h2 className={`${compact ? 'text-base' : 'text-xl md:text-2xl'} font-bold truncate`}>{capitalizeName(cliente.nome)}</h2>
                  <button
                    onClick={async () => {
          ...
          <div
            className={
              compact
                ? 'flex-1 min-h-0 overflow-y-auto px-3 py-3 text-sm'
                : isMobile
                ? 'flex-1 min-h-0 overflow-y-auto px-4 pt-4'
                : 'flex-1 min-h-0 overflow-y-auto px-6 py-4'
            }
            style={!compact && isMobile ? { paddingBottom: 'calc(1rem + env(safe-area-inset-bottom))' } : undefined}
          >

            {editMode ? (
              <div className={`${compact ? 'space-y-3 mb-4' : 'space-y-4 mb-6'}`}>
              ...
              <div className={`${compact ? 'space-y-3 mb-4' : 'space-y-4 mb-6'}`}>
                ...
                <div className={`flex gap-2 ${compact ? 'flex-col items-stretch' : 'items-center'}`}>
                  <button
            ...
            <div className={`${compact ? 'mb-4 p-3' : 'mb-6 p-4'} bg-blue-50 rounded-lg`}>
              <div className="flex justify-between items-center mb-3">
                <h3 className={`${compact ? 'text-sm' : ''} font-bold flex items-center gap-2`}>
                  📅 Agendamento de Contato
                </h3>
                {agendamento && (
                  <div className={`flex gap-2 ${compact ? 'flex-col items-end' : ''}`}>
                    <button
            ...
            <div className="border-t pt-4">
              ...
              <div className="space-y-3">
                <textarea
                  ...
                  className={`w-full border rounded px-3 py-2 ${compact ? 'h-16' : 'h-24'}`}
                />
                <div className={`flex gap-2 ${compact ? 'flex-col' : ''}`}>
                  <select
